...
def handle_pack(reponame_git_path: str):
    ...
    pack_file_names = [name for name in os.listdir(pack_folder) if name.endswith('.pack')]
    if len(pack_file_names) > 0:
        for pack_file_name in pack_file_names:
            idx_file_name = pack_file_name[:-4] + "idx"
            object_idx = createObjectIdx(filepath=os.path.join(pack_folder, idx_file_name))
            object_pack = createObjectPack(object_idx=object_idx, filepath=os.path.join(pack_folder, pack_file_name))
            print("pause")
    else:
        return
...
def handle_repositories(repositories_path: str):
    ...
    ownername_paths = [f.path for f in os.scandir(repositories_path) if f.is_dir()]
    for ownername_path in ownername_paths:
        # iterate all the repositories
        reponame_git_paths = [f.path for f in os.scandir(ownername_path) if f.is_dir()]
...
def main():
    ...
    if config is None:
        print("Error: configuration file {config_path} not found".format(config_path=config_path))
        sys.exit(1)
