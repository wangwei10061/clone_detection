    ...
    with io.open(filepath, 'rb') as f:
        ...
        if content[:4] == b"\377tOc":
            ...
            if version == 2:
                ...
                raise KeyError("Version is not supported {version}".format(version=version))
        else:
...
class ObjectIdxV1(ObjectIdx):
    def __init__(self, filepath, content):
        ...
        self.object_num = int(self.content[offset_index:offset_index+256*4][-4:].hex(), 16)
        offset_index += 256*4
        ...
        self.object_names, self.object_offsets = self.extract_entries(self.content[offset_index:offset_index+24*self.object_num])
...
class ObjectIdxV2(ObjectIdx):
    def __init__(self, filepath, content):
        ...
        self.object_names = self.extract_names(self.content[offset_index:offset_index+20*self.object_num])
    ...
    def extract_names(self, content_names):
        return [content_names[i:i + 20].hex() for i in range(0, len(content_names), 20)]
    ...
    def extract_object_offsets(self, offset_offset_index):
        ...
        result = []
        small_offset_content = self.content[offset_offset_index:large_offset_offset_index]
        for i in range(0, len(small_offset_content), 4):
            offset = struct.unpack_from(">L", small_offset_content, i)[0]
            if offset >= 2 ** 31:
                offset = struct.unpack_from(">Q", self.content, large_offset_offset_index + (offset & (2 ** 31 - 1)) * 8)[0]
